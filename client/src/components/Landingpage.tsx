...
import { ArrowRight, Shirt, Zap, Star, Heart, ShoppingCart, Eye, Sparkles, ShoppingBag } from "lucide-react"
...
export default function Landingpage() {
  ...
  const getBadgeColor = (badge:any) => {
    switch (badge) {
      ...
      case "New Arrival": return "bg-gradient-to-r from-green-400 to-emerald-400"
      case "Premium": return "bg-gradient-to-r from-purple-400 to-indigo-400"
      case "Trending": return "bg-gradient-to-r from-blue-400 to-cyan-400"
      case "Sport": return "bg-gradient-to-r from-red-400 to-pink-400"
      case "Limited": return "bg-gradient-to-r from-gray-400 to-gray-600"
      default: return "bg-gradient-to-r from-purple-400 to-indigo-400"
    }
  }
      ...
      <div className="relative z-10 container mx-auto px-4 py-20">
        ...
        >
          ...
          <p className="text-xl md:text-2xl max-w-4xl mx-auto mb-12 text-gray-300 font-light leading-relaxed">
            Curated collections of <span className="font-semibold text-white">premium clothing</span> designed to{" "}
        ...
        {/* Products Grid */}
        <div
          className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-16 transition-all duration-1000 delay-400 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
          }`}
        >
          {clothingProducts.map((product, index) => (
            <Card
              key={product.id}
              className="group bg-purple-900/20 border-purple-400/30 backdrop-blur-sm hover:bg-purple-900/40 transition-all duration-500 hover:scale-105 hover:shadow-2xl hover:shadow-purple-500/25 cursor-pointer overflow-hidden"
            ...
            >
              <CardContent className="p-0">
                ...
                <div className="relative overflow-hidden">
                  ...
                  <div className={`absolute inset-0 bg-black/60 flex items-center justify-center space-x-3 transition-opacity duration-300 ${
                    ...
                    <Button 
                      size="sm" 
                      variant="outline"
                      className="border-purple-400/40 bg-purple-900/40 text-white hover:bg-purple-900/60 backdrop-blur-sm"
                    >
                  ...
                  {/* Badge */}
                  <div className={`absolute top-4 left-4 px-3 py-1 ${getBadgeColor(product.badge)} rounded-full`}>
                  ...
                  <button
                    ...
                    className="absolute bottom-4 right-4 p-3 bg-purple-900/40 backdrop-blur-sm rounded-full hover:bg-purple-900/60 transition-colors border border-purple-400/30"
                  >
                    <Heart 
                ...
                <div className="p-6">
                  ...
                  <h3 className="text-xl font-bold text-white mb-4 line-clamp-2 group-hover:bg-gradient-to-r group-hover:from-purple-400 group-hover:to-indigo-400 group-hover:bg-clip-text group-hover:text-transparent transition-all duration-300">
                    {product.name}
                  </h3>
                  ...
                  <div className="flex items-center space-x-2 mb-4">
                    ...
                    <div className="flex space-x-2">
                      {product.colors.slice(0, 3).map((color, colorIndex) => (
                        <div
                          key={colorIndex}
                          className="w-5 h-5 rounded-full border-2 border-purple-400/30 hover:border-purple-400 transition-colors cursor-pointer"
                          style={{
                            backgroundColor: color.toLowerCase() === 'white' ? '#ffffff' :
                                           ...
                                           color.toLowerCase() === 'burgundy' ? '#881337' :
                      ...
                      {product.colors.length > 3 && (
                        <div className="text-gray-400 text-xs flex items-center">+{product.colors.length - 3}</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button 
                      size="sm" 
                      className="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-bold transform hover:scale-105 transition-all duration-300 shadow-lg shadow-purple-500/25"
                    >
